...
| ~~UserPromptSubmit hook 注入 `additionalContext`~~ | ~~❌~~ → ✅ | ~~command 型 hook 的 additionalContext 不生效~~ — **早期誤判，後來驗證可行，見 Step 7** |
...
- ~~「Hook 終究是 Hook，additionalContext 只能看著。」~~ *（早期版本的評論，後來驗證 command 型 hook 可注入 additionalContext，見 Step 7）*
- 「『看完即焚』的設計，就是故意的吧。」
